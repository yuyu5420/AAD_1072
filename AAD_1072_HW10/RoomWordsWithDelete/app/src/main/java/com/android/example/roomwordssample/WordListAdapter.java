...
public class WordListAdapter extends RecyclerView.Adapter<WordListAdapter.WordViewHolder> {
    ...
    public void onBindViewHolder(WordViewHolder holder, int position) {
        if (mWords != null) {
            ...
            String output_data = current.getWord();
            if (output_data.indexOf('+') >= 0) {
                String question = output_data.substring(0, output_data.indexOf('+'));
                String answer = output_data.substring(output_data.indexOf('+') + 1);
                holder.wordItemView.setText(question);
                holder.answerView.setText(answer);
            }
            else holder.wordItemView.setText(current.getWord());
        } else {
